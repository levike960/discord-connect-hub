...
def register_fraction_routes(app, db, models):
    ...
    def fraction_preorder():
        ...
        if request.method == "POST":
            ...
            if items_list:
                db.session.add(StockMovement(
                    ...
                    quantity=total_cost,
                    reason=f"Előrendelés: {', '.join(items_list)}",
        ...
        for sm in preorder_logs_raw:
            ...
            if sm.data_json:
                try:
                    data = json.loads(sm.data_json)
                    items_text = ", ".join(
                        f"{i['qty']} {i['unit']} {i['name']}" for i in data.get("items", [])
                    )
                    preorder_logs.append({
                        "user": sm.user,
                        "items": items_text,
                        "total_cost": data.get("total_cost", sm.quantity),
