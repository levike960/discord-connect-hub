...
DISCORD_CLIENT_ID = 1474158475150885127 #os.environ.get("DISCORD_CLIENT_ID", "")
DISCORD_CLIENT_SECRET = "HKBx_WOdAIZ7Zl2XHsTH2Mup3DGi4E2T" #os.environ.get("DISCORD_CLIENT_SECRET", "")
DISCORD_REDIRECT_URI = os.environ.get(
    "DISCORD_REDIRECT_URI", "https://casadivino.hu/callback"
)
...
import os
from seed import export_seed, restore_seed

CONTROL_FILE = "seed_control.txt"


def check_seed_control():
    ...
    with app.app_context():
        if config.get("Export Seed") == "true":
            print("⚙ Export seed indul...")
            export_seed()

        if config.get("Restore") == "true":
            print("⚙ Restore seed indul...")
            restore_seed()

    # 🔄 reseteljük automatikusan
    with open(CONTROL_FILE, "w", encoding="utf-8") as f:
        f.write("Export Seed = False\n")
        f.write("Restore = False\n")


# App induláskor lefut
check_seed_control()

if __name__ == "__main__":
    app.run(host="0.0.0.0", port=9812, debug=True)
